...
/*List devuelve una lista de usuarios*/
func List(ID string, page int64, search string, tipo string) ([]*models.Usuario, bool) {
	...
	query := bson.M{
		"nombre": bson.M{"$regex": `(?i)` + search}, // i siginifica que no se fija si es mayuscula o minus
	}
	...
	for cur.Next(ctx) {
		...
		encontrado, err = relacion.ListByUser(r)
